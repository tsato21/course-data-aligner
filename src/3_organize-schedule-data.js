...
function organizedClassScheduleDataType3() {
  ...
  let programDataSheetName = Browser.inputBox(
    `Input the name of the sheet where you pasted the class schedule data provided by the Program (e.g., 3_Schedule Data by Program).`,
    Browser.Buttons.OK_CANCEL
  );
  let programDataSheetNameTrimmed = programDataSheetName.trim();
  let programDataSheet = spreadSheet.getSheetByName(
    programDataSheetNameTrimmed
  );
  ...
  let displaySheetName = Browser.inputBox(
    `Input the name of the sheet where organized course offering data is displayed (e.g., 3_Organized Schedule Data).`,
    Browser.Buttons.OK_CANCEL
  );
  ...
  if (displaySheet === null) {
    Browser.msgBox(
      "Sheet with the name '" + displaySheetNameTrimmed + "' does not exist."
    );
    return;
  }
  ...
  let semester = Browser.inputBox(
    `【★】Input Semester (Either 'S' for Spring Semester or 'F' for Fall Semester)`
  );
  ...
  for (let i = 3; i < data.length; i++) {
    ...
    for (let j = 1; j < row.length; j++) {
      ...
      console.log(
        `Target record is as follows: courseCode is ${courseCode}; cellColumn is ${cellColumn}, sectionCode is ${
          sectionCodes[j - 1]
        }, faculty is ${faculty}, startTime is ${startTime}, endTime is ${endTime}, room is ${room}`
      );

      processCourseRecord_(
        courseCode,
        cellColumn,
        rawValue,
        sectionCodes[j - 1],
...
function processCourseRecord_(
  ...
  if (courseCode == 'ADV150') {
    //If the courseCode is "BRI150", set classCode and push the record individually
  ...
  } else if (slashIncludeCourse) {
    //If the courseCode includes "/", split by "/" and push two records with some adjustment
    ...
    console.log(
      `${slashIncludeCourse} is splitted by "/" and courseCode is ${classCode_1} and ${classCode_2}.`
    );
    ...
    //If the courseCode is included in "EAA_COURSES" array, push the record with some adjustment
    ...
    // If the courseCode is a regular one, push the record with some adjustment
...
function pushCourseRecord_(
  ...
  let daysOfWeek = [];
  let secondRecordRemoveCourse = SECOND_RECORD_REMOVE_COURSES.some((course) =>
    classCode.includes(course)
  );
  if (cellColumn >= MON_WED_COL_START && cellColumn <= MON_WED_COL_END) {
    daysOfWeek = ['月', '水'];
    console.log(
      `Since this cell(${cellColumn}) is in Monday/Wednesday range (${MON_WED_COL_START}:${MON_WED_COL_END}), record for Monday is pushed.`
    );
    ...
    daysOfWeek = ['火', '木'];
    console.log(
      `Since this cell(${cellColumn}) is in Tuesday/Thursday range(${TUE_THU_COL_START}:${TUE_THU_COL_END}), record for Tuesday is pushed.`
    );
    if (secondRecordRemoveCourse) {
      daysOfWeek = ['木'];
    ...
    daysOfWeek = ['金'];
    console.log(
      `Since this cell(${cellColumn}) is in Friday range (${FRI_COL_START}:${FRI_COL_END}), record for Friday is pushed.`
    );
  } else {
    daysOfWeek = ['範囲外'];
    console.log(
      `The cell column of the record, ${rawValue} is ${cellColumn}, which is not within any of the designated ranges. Record as exception is pushed`
    );
  }
...
function displayRecords_(records, header, displaySheet) {
  ...
  for (let i = 0; i < records.length; i++) {
    if (!courseCodePattern.test(records[i][1]) || records[i][1] == '') {
      highlightCells_(
        ...
        2,
        records[i][1],
        'not valid for course code',
        displaySheet
      );
    } else if (
      HIGHLIGHT_COURSES.some((course) => records[i][1].includes(course))
    ) {
      highlightCells_(
        i + 2,
        2,
        records[i][1],
        'a course that needs to be checked',
        displaySheet
      );
    }
    ...
    if (!dayPattern.test(records[i][3]) || records[i][3] == '') {
      highlightCells_(
        ...
        4,
        records[i][3],
        'not valid for days of week',
        displaySheet
      );
    }
    if (
      !timePattern.test(records[i][4]) ||
      !timePattern.test(records[i][5]) ||
      records[i][4] == '' ||
      records[i][5] == ''
    ) {
  ...
  Browser.msgBox(
    `Course data is output in ${sheetName} in the designated structure along with designated faculty names replaced.`
  );
}
...
function highlightCells_(rowNum, colNum, value, highlightReason, displaySheet) {
  displaySheet
    ...
    .setFontWeight('bold');
  console.log(
    `The cell is colored in red since ${value} is ${highlightReason} or just empty.`
  );
}
