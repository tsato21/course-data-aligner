const FACULTY_NAME_REPLACEMENT = [{ original: 'LUHFE', replace: 'LL' }];
/**
...
function organizedClassScheduleDataType2() {
  ...
  let programDataSheetName = Browser.inputBox(
    `Input the name of the sheet where you pasted the class schedule data provided by the Program (e.g., 2-2_Schedule Data by Program).`,
  ...
  let displaySheetName = Browser.inputBox(
    `Input the name of the sheet where organized course offering data is displayed (e.g., 2-2_Organized Schedule Data).`,
    Browser.Buttons.OK_CANCEL
  );
  ...
  for (let i = 0; i < intermediateData.length; i++) {
    ...
    let key =
      intermediateData[i].courseCode +
      intermediateData[i].day +
      '_' +
      intermediateData[i].faculty;
  ...
  Browser.msgBox(
    'Course data by the Program is displayed (Sorted by Course Code & Day).'
  );
}
