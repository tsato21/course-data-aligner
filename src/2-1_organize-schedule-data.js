...
function organizedClassScheduleDataType1() {
  ...
  let programDataSheetName = Browser.inputBox(
    `Input the name of the sheet where you pasted the class schedule data provided by the Program (e.g., 2-1_Schedule Data by Program).`,
  ...
  let displaySheetName = Browser.inputBox(
    `Input the name of the sheet where organized course offering data is displayed (e.g., 2-1_Organized Schedule Data).`,
    Browser.Buttons.OK_CANCEL
  );
  ...
  for (let key in groupedData) {
    ...
    if (records.length > 1) {
      ...
      if (
        records.some((record) => record.classNumber === 2) &&
        records.some((record) => record.classNumber === 3)
      ) {
  ...
  if (exceptionalCells.length > 0) {
    ...
    displaySheet
      .getRange(3, 7, exceptionalCells.length, 1)
      .setValues(exceptionalCells);
  }

  Browser.msgBox(
    'Course data by the Program is displayed as follows: Usual Courses are in A~E column, being sorted by Course Code & Day; Exceptional Courses are in J column. '
  );
}
