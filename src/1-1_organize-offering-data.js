...
function organizedCourseOfferingDataType1() {
  let programDataSheetName = Browser.inputBox(
    `Input the name of the sheet where you pasted the course offering data provided by the Program (e.g., 1-1_Offering Data by Program).`,
  ...
  let displaySheetName = Browser.inputBox(
    `Input the name of the sheet where organized course offering data is displayed (e.g., 1-1_Organized Offering Data).`,
    Browser.Buttons.OK_CANCEL
  );
  ...
  try {
    for (let i = 1; i < data.length; i++) {
      ...
      for (let j = 1; j < data[i].length; j++) {
        ...
        console.log(
          `Course Code is ${courseCode} and semester is ${offeringSemester}`
        );
    ...
    let displaySheet =
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(displaySheetName);
